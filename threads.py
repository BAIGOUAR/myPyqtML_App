...
from train import *
from  preprocessing_utils import PreprocessDataset

class Train_Model_WorkerSignals(QtCore.QObject):
    finished = QtCore.pyqtSignal(object, object)


class Train_Model_Thread(QtCore.QRunnable, datasetTrainer):
    """
    Worker thread
...
class Load_Dataset_Thread(QtCore.QRunnable):
    ...
    def run(self):
        ...
        if return_code == 'sucess':
            ...
            #Fillings all the columns of the dataset to be ploted
            ...
            self.signals.display_message.emit(QtWidgets.QMessageBox.Warning, 'Error',
                                              'Invalid Input format, accepted extensions are (csv, xls, xslx). \nTry Excel or .csv format',
                                              'Error')


        elif return_code == 'exception_in_the_file':
            self.signals.display_message.emit(QtWidgets.QMessageBox.Warning, 'Error', 'Invalid Input File, accepted extensions are (.csv, .xslx, .xls)', 'Error')
...
class Pre_Process_Dataset_Thread(QtCore.QRunnable):
    ...
    def replaceValues(self, item_data):
        target_variable = item_data['variable']
        new_value = item_data['new_values']
        old_values = item_data['old_values']
        self.ml_model.replace_values(target_variable, new_value, old_values)

    def applyFiltering(self, item_data):
        ...
        self.ml_model.filter_out_values(filtering_variable, filtering_value, filtering_operator)

    def preProcessingAction(self, item_data):
        preprocessData  = PreprocessDataset()
        switcher = {
            'rm_duplicate_rows': preprocessData.remove_duplicate_rows(),
            'rm_constant_var'  : preprocessData.remove_constant_variables(),
            'apply_num_scaling': preprocessData.scale_numeric_values(),
            'rm_outliers'      : preprocessData.remove_outliers(item_data['cut_off']),
            'replace_values'   : self.replaceValues(item_data),
            'apply_filtering'  : self.applyFiltering(item_data),
        }
        return switcher.get(item_data['pre_processing_action'], "Invalid preprocessing action")
    ...
    def run(self):
        ...
        for i in range(listwidget.count()):
            ...
            self.preProcessingAction(item_data)
    ...
    def run(self):
        ...
        if target_widget.objectName() == 'dataVisualisePlot_widget':
            ...
            if content['is_regression']:
                ...
                target_widget.canvas.figure.tight_layout()
